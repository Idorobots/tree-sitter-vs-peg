...
(define (bench f inputs)
  (define (median-of n f)
    (let loop ((i n)
               (acc '()))
      (if (= i 0)
          (list-ref (sort acc <)
...
(define (run-bench filename sizes)
  ...
  (let ((test-files (map (lambda (s)
                           (input filename s))
                         sizes)))
    (display "# lines:       ")
    (display-result (map (lambda (f)
                           (length (string-split f "\n")))
                         test-files))

    (display "TreeSitter:    ")
    (display-result (bench parse-ts test-files))

    (display "PEG generator: ")
    (display-result (bench parse-pg test-files))))
